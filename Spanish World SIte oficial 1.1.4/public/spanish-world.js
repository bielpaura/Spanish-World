...
async function entrarPlataforma(user) {
  usuarioAtual = user;
  ...
  inicializarHeroTematico();
  await esperarHeroPronto();

  const auth = document.getElementById('authPage');
  const site = document.getElementById('site');
  auth?.classList.add('saindo');
  site?.classList.add('entrando');
  site?.classList.remove('oculto');
  setTimeout(() => {
    auth?.classList.add('oculto');
    site?.classList.remove('entrando');
    auth?.classList.remove('saindo');
  }, 420);

  setTimeout(() => {
    atualizarUIUsuario();
    navegar('inicio');
    ampliarCatalogoDeAulas();
    gerarTodasLecciones();
    gerarExercicios();
    gerarTopicos();
    gerarFrasesPronuncia();
    gerarFeedbacks();
    gerarRanking();
    verificarStreak();
...
function inicializarHeroTematico() {
  const slideWrap = document.getElementById('heroSlideshow');
  const themeWrap = document.getElementById('heroThemes');
  if (!slideWrap || slideWrap.dataset.ready === 'true') return;

  slideWrap.innerHTML = heroImagens.map((img, i) =>
    `<div class="hero-slide ${i === 0 ? 'ativo' : ''}" data-hero-slide="${i}" style="background-image:url('${img.url}')"></div>`
  ).join('');

  if (themeWrap) {
    themeWrap.innerHTML = heroImagens.map((img, i) =>
      `<button class="hero-theme-btn ${i === 0 ? 'ativo' : ''}" onclick="trocarHeroTema(${i}, true)">${img.tag}</button>`
    ).join('');
  }

  slideWrap.dataset.ready = 'true';
  heroImagens.forEach(img => {
...
function iniciarHeroRotacao() {
  clearInterval(heroTimer);
  heroTimer = setInterval(() => trocarHeroTema((heroSlideAtual + 1) % heroImagens.length, false), 6500);
}

function trocarHeroTema(indice, manual = false) {
  const slides = document.querySelectorAll('.hero-slide');
  const botoes = document.querySelectorAll('.hero-theme-btn');
  if (!slides.length) return;
  heroSlideAtual = indice;
  slides.forEach((s, i) => s.classList.toggle('ativo', i === indice));
  botoes.forEach((b, i) => b.classList.toggle('ativo', i === indice));
  if (manual) iniciarHeroRotacao();
}
...
function navegar(secao) {
  ...
  if (secao === 'aulas-pronuncia') gerarFrasesPronuncia();
  if (secao === 'feedbacks') gerarFeedbacks();
  if (secao === 'inicio') inicializarHeroTematico();
  if (secao === 'digitacao') renderizarDigitacaoSetup();
}
...
function toggleMobileMenu() {
  ...
  if (!nav) return;
  if (mobileMenuAberto) {
    nav.style.cssText = `display:flex;flex-direction:column;position:fixed;top:72px;left:14px;width:min(300px,calc(100vw - 28px));background:#140707;padding:14px;z-index:999;gap:6px;border:1px solid rgba(255,255,255,0.12);border-radius:18px;box-shadow:0 18px 44px rgba(0,0,0,0.45);align-items:stretch;`;
...
const heroImagens = [
  {
    nome: 'Alhambra',
    tag: '🏰 Patrimônio',
    url: 'https://commons.wikimedia.org/wiki/Special:FilePath/Alhambra%20from%20Generalife%20%282017%29.jpg'
  },
  {
    nome: 'Madrid',
    tag: '🏙️ Cidade',
    url: 'https://commons.wikimedia.org/wiki/Special:FilePath/Madrid,%20Plaza%20Mayor.jpg'
  },
  {
    nome: 'Paella',
    tag: '🥘 Gastronomia',
    url: 'https://commons.wikimedia.org/wiki/Special:FilePath/Cooking%20a%20paella.jpg'
  },
  {
    nome: 'Plaza Mayor',
    tag: '☀️ Cultura',
    url: 'https://commons.wikimedia.org/wiki/Special:FilePath/Plaza%20Mayor%20de%20Madrid%20-%2001.jpg'
  }
];

const typingPersonagens = [
  { nome: 'Sol', icon: '☀️' },
  { nome: 'Luna', icon: '🌙' },
  { nome: 'Estrella', icon: '⭐' },
  { nome: 'Rayo', icon: '⚡' }
...
const typingFrases = {
  iniciante: [
    ...
    'Vivo en Brasil y estudio todos los días.',
    'Gracias por ayudarme.'
  ],
  intermediario: [
    'Me gustaría viajar por España el próximo año.',
    'Necesito practicar la pronunciación con más calma.',
    'En el restaurante quiero pedir una paella y agua.',
    'Estoy aprendiendo nuevas palabras para conversar mejor.',
    'La cultura hispánica tiene música, historia y mucha tradición.'
  ],
  avancado: [
    'Aunque el camino parezca difícil, la práctica diaria mejora la fluidez.',
    'Para expresarme con naturalidad, debo escuchar, repetir y corregir mis errores.',
    'El español profesional exige claridad, cortesía y vocabulario preciso.',
    'Cuando viajo, prefiero hablar con la gente local para aprender expresiones reales.',
    'La confianza aparece cuando transformamos el estudio en una rutina constante.'
  ]
};
...
function preloadHeroImagens() {
  if (!heroPreloadPromise) {
    heroPreloadPromise = Promise.all(heroImagens.map(img => carregarImagem(img.url)));
  }
  return heroPreloadPromise;
}

function esperarHeroPronto() {
  return Promise.race([
    preloadHeroImagens(),
...
let catalogoAmpliado = false;
function ampliarCatalogoDeAulas() {
  if (catalogoAmpliado) return;
  const extrasPorNivel = {
    a1: ['Cumprimentos formais', 'Meu primeiro diálogo', 'Pronomes demonstrativos', 'Perguntas essenciais', 'Mini leitura A1', 'Ditado guiado A1'],
    a2: ['Pretérito perfecto', 'Planos de fim de semana', 'Pedidos educados', 'Conectores simples', 'Compras online', 'Mini entrevista A2'],
    b1: ['Opiniões com clareza', 'Narrar experiências', 'Conselhos e sugestões', 'Podcast lento B1', 'Debate cotidiano', 'Resumo de notícia'],
    b2: ['Argumentação avançada', 'Reunião profissional', 'Texto opinativo', 'Espanhol acadêmico', 'Apresentação oral', 'Preparação DELE B2'],
    c1: ['Ironia e humor', 'Ensaio crítico', 'Registro culto', 'Análise literária', 'Podcast nativo C1', 'Mediação linguística'],
    c2: ['Retórica profissional', 'Tradução simultânea', 'Sotaques complexos', 'Expressões raras', 'Debate de alto nível', 'Exame oral C2']
  };
  Object.entries(extrasPorNivel).forEach(([nivel, titulos]) => {
    titulos.forEach((titulo, idx) => {
      todasLecciones[nivel].push({
        id: `${nivel}-extra-${idx + 1}`,
        emoji: ['✨','🎧','🧠','🗣️','📘','🏁'][idx] || '📚',
        titulo,
        desc: 'Aula profissional com prática, exemplos e revisão aplicada',
        dif: nivel === 'a1' || nivel === 'a2' ? 'medio' : 'dificil',
        pts: nivel === 'a1' ? 20 : nivel === 'a2' ? 25 : nivel === 'b1' ? 30 : 45
      });
    });
  });
  const extrasTema = {
    pronuncia: ['Ritmo natural', 'Som de J e G', 'Seseo e ceceo', 'Leitura em voz alta', 'Sombras de fala'],
    gramatica: ['Uso de lo', 'Perífrasis verbais', 'Orações relativas', 'Concordância avançada'],
    vocabulario: ['Falsos cognatos', 'Colocações naturais', 'Vocabulário emocional', 'Vocabulário para reuniões'],
    negocios: ['Pitch em espanhol', 'Negociação difícil', 'Relatório executivo', 'LinkedIn em espanhol'],
    cultura: ['Mapa dos sotaques', 'Etiqueta social', 'Festas populares', 'História latino-americana']
  };
  Object.entries(extrasTema).forEach(([tema, titulos]) => {
    titulos.forEach((titulo, idx) => todasLecciones[tema].push({
      id: `${tema}-extra-${idx + 1}`,
      emoji: '🌟',
      titulo,
      desc: 'Conteúdo extra para aprofundar sua fluência',
      dif: idx > 1 ? 'dificil' : 'medio',
      pts: idx > 1 ? 35 : 25
    }));
  });
  catalogoAmpliado = true;
}
...
function iniciarQuiz(tema) {
  ...
  quizAtual = {
    tema, perguntas: [...perguntas].sort(() => Math.random() - 0.5).slice(0, 12).map(prepararPerguntaQuiz),
...
function renderizarQuiz() {
  ...
  container.innerHTML = `
    ...
    <div class="quiz-pergunta-card">
      <div class="quiz-visual" id="qVisual">
        <img class="quiz-visual-img" id="qVisualImg" alt="Imagem temática do quiz">
        <div class="quiz-visual-shade"></div>
        <div class="quiz-visual-emoji" id="qVisualEmoji">🎯</div>
        <div>
          <div class="quiz-visual-title" id="qVisualTitle">Quiz interativo</div>
          <div class="quiz-visual-tip" id="qVisualTip">Observe a imagem temática e escolha a melhor resposta.</div>
        </div>
      </div>
    ...
    <div class="quiz-mascote"><span>🎓</span><p id="quizMascoteMsg">Respire, leia com calma e confie no que você estudou.</p></div>
  `;
...
function mostrarPerguntaAtual() {
  ...
  const visual = obterVisualQuiz(q, quizAtual.tema);
  document.getElementById('qVisualEmoji').textContent = visual.emoji;
  document.getElementById('qVisualTitle').textContent = visual.titulo;
  document.getElementById('qVisualTip').textContent = visual.dica;
  document.getElementById('qVisual').style.background = visual.bg;
  const visualImg = document.getElementById('qVisualImg');
  if (visualImg) {
    visualImg.src = visual.img;
    visualImg.alt = visual.titulo;
  }
  const mascote = document.getElementById('quizMascoteMsg');
  if (mascote) mascote.textContent = visual.mascote;
...
function prepararPerguntaQuiz(pergunta) {
  const base = { ...pergunta, o: [...pergunta.o] };
  const correta = base.o[base.c];
  const banco = ['No sé', 'Tal vez', 'Por supuesto', 'Nunca', 'Siempre', 'Ahora', 'Muy bien', 'Hasta luego', 'Con permiso', 'A menudo'];
  banco.forEach(op => {
    if (base.o.length < 4 && op !== correta && !base.o.includes(op)) base.o.push(op);
  });
  const embaralhadas = base.o.map((op, i) => ({ op, correta: i === base.c })).sort(() => Math.random() - 0.5);
  return { ...base, o: embaralhadas.map(item => item.op), c: embaralhadas.findIndex(item => item.correta) };
}

function obterVisualQuiz(q, tema) {
  const texto = `${q.p} ${q.o.join(' ')}`.toLowerCase();
  const visuais = [
    { k:['bom dia','buenos','hola','adiós','adios','gracias'], emoji:'🌅', titulo:'Saudações em Madrid', dica:'Pense no contexto da fala: chegada, despedida ou agradecimento.', bg:'linear-gradient(135deg,#ef4444,#f97316,#facc15)', img:'https://commons.wikimedia.org/wiki/Special:FilePath/Madrid,%20Plaza%20Mayor.jpg' },
    { k:['comida','pão','pan','agua','leche','restaurante','queso'], emoji:'🍽️', titulo:'Gastronomia espanhola', dica:'Imagine a cena: cardápio, mesa e pedido.', bg:'linear-gradient(135deg,#dc2626,#fb923c,#fde047)', img:'https://commons.wikimedia.org/wiki/Special:FilePath/Cooking%20a%20paella.jpg' },
    { k:['número','numero','cuatro','cinco','mil','treinta','quince'], emoji:'🔢', titulo:'Números na cidade', dica:'Leia cada alternativa em voz baixa antes de marcar.', bg:'linear-gradient(135deg,#7c2d12,#ea580c,#facc15)', img:'https://commons.wikimedia.org/wiki/Special:FilePath/Plaza%20Mayor%20de%20Madrid%20-%2001.jpg' },
    { k:['familia','família','mãe','padre','madre','irmão','hermana'], emoji:'👨‍👩‍👧', titulo:'Vida cotidiana', dica:'Procure semelhanças com o português, mas cuidado com falsos amigos.', bg:'linear-gradient(135deg,#be123c,#f97316,#fcd34d)', img:'https://commons.wikimedia.org/wiki/Special:FilePath/Madrid,%20Plaza%20Mayor.jpg' },
    { k:['cor','azul','rojo','verde','amarillo','blanco','negro'], emoji:'🎨', titulo:'Cores da Espanha', dica:'Associe a palavra a uma imagem mental colorida.', bg:'linear-gradient(135deg,#dc2626,#f59e0b,#22c55e)', img:'https://commons.wikimedia.org/wiki/Special:FilePath/Alhambra%20from%20Generalife%20%282017%29.jpg' },
    { k:['animal','perro','gato','pájaro','caballo','oso'], emoji:'🦁', titulo:'Vocabulário visual', dica:'A pronúncia ajuda: perro tem R forte.', bg:'linear-gradient(135deg,#991b1b,#f97316,#84cc16)', img:'https://commons.wikimedia.org/wiki/Special:FilePath/Alhambra%20from%20Generalife%20%282017%29.jpg' }
  ];
  const achado = visuais.find(v => v.k.some(k => texto.includes(k)));
  const padrao = { emoji:'🧠', titulo: tema.charAt(0).toUpperCase() + tema.slice(1), dica:'Use eliminação: descarte o que claramente não combina.', bg:'linear-gradient(135deg,#b91c1c,#f97316,#facc15)', img:'https://commons.wikimedia.org/wiki/Special:FilePath/Alhambra%20from%20Generalife%20%282017%29.jpg' };
  const visual = achado || padrao;
  return { ...visual, mascote: 'Dica do mascote: escolha primeiro pelo sentido, depois confira a gramática.' };
}
...
async function enviarMsg() {
  ...
  const respostaLocal = gerarRespostaSofia(texto);
  chatHistorico.push({ role: 'assistant', content: respostaLocal });
  adicionarMsgChat('bot', respostaLocal);
  const userMsgsLocal = chatHistorico.filter(m => m.role === 'user').length;
  if (userMsgsLocal >= 5 && !conquistas.includes('chat_5')) darConquista('chat_5', '💬 5 mensagens enviadas!');
  if (userMsgsLocal >= 20 && !conquistas.includes('chat_master')) darConquista('chat_master', '🗣️ Conversador — 20 mensagens!');
  adicionarPontos(2);
  salvarUsuario();
...
function gerarRespostaSofia(texto) {
  const msg = texto.toLowerCase();
  const nome = usuarioAtual?.nome || 'aluno';
  const nivel = traduzirNivel(usuarioAtual?.nivel);
  const correcoes = [
    { errado: 'yo hablar', certo: 'yo hablo', dica: 'No presente, "hablar" vira "hablo" com "yo".' },
    { errado: 'yo soy cansado', certo: 'yo estoy cansado', dica: 'Estados temporários usam "estar".' },
    { errado: 'me llamo es', certo: 'me llamo', dica: 'Para dizer o nome, use só "me llamo Ana" ou "mi nombre es Ana".' },
    { errado: 'gracias para', certo: 'gracias por', dica: 'Agradecimento por motivo usa "por".' }
  ];
  const encontrada = correcoes.find(c => msg.includes(c.errado));
  if (encontrada) {
    return `Boa tentativa, ${nome}! Uma forma mais natural seria: **${encontrada.certo}**.\n\n${encontrada.dica}\n\nAgora pratique assim: **Yo hablo español todos los días.** Quer tentar criar outra frase?`;
  }
  if (msg.includes('restaurante') || msg.includes('comida') || msg.includes('pedir')) {
    return `Vamos praticar restaurante no seu nível ${nivel}.\n\nFrase útil: **Quisiera una mesa para dos, por favor.** (Eu gostaria de uma mesa para dois, por favor.)\n\nResponda em espanhol: você quer pedir água, café ou uma sobremesa?`;
  }
  if (msg.includes('viagem') || msg.includes('aeroporto') || msg.includes('hotel')) {
    return `Perfeito para viagem. Use este mini diálogo:\n\n**Hola, tengo una reserva a nombre de ${nome}.**\n**¿A qué hora es el check-in?**\n\nSua missão: escreva uma frase perguntando onde fica a estação de metrô.`;
  }
  if (msg.includes('pron') || msg.includes('sotaque') || msg.includes('fala')) {
    return `Para pronúncia, foque em vogais limpas: **a, e, i, o, u** quase não mudam de som.\n\nTreino rápido: diga devagar **pero** e depois **perro**. O segundo precisa de vibração mais forte no R.\n\nVocê também pode abrir a aba **Pronúncia** e usar o laboratório de fala.`;
  }
  if (msg.includes('verbo') || msg.includes('conjugar') || msg.includes('gram')) {
    return `Vamos simplificar. No presente, verbos terminados em **-ar** ficam assim com **hablar**:\n\n**yo hablo, tú hablas, él habla, nosotros hablamos, ellos hablan**.\n\nComplete: **Yo ____ español con mis amigos.**`;
  }
  if (msg.includes('trabalho') || msg.includes('negócio') || msg.includes('negocio') || msg.includes('reuni')) {
    return `Espanhol profissional:\n\n**Necesito revisar la propuesta antes de la reunión.** (Preciso revisar a proposta antes da reunião.)\n\nPalavras-chave: **reunión** (reunião), **plazo** (prazo), **presupuesto** (orçamento), **acuerdo** (acordo). Quer simular uma reunião comigo?`;
  }
  if (msg.includes('apresent') || msg.includes('meu nome') || msg.includes('llamo')) {
    return `Claro. Modelo profissional e natural:\n\n**Hola, me llamo ${nome}. Soy de Brasil y estoy aprendiendo español para comunicarme mejor.**\n\nAgora personalize: troque o objetivo final por algo seu, como viagem, trabalho ou estudos.`;
  }
  const desafios = [
    'Escreva uma frase com **quiero** e eu corrijo.',
    'Traduza para espanhol: "Eu estudo espanhol todos os dias."',
    'Responda em espanhol: **¿Qué hiciste hoy?**',
    'Monte uma pergunta usando **dónde**.'
  ];
  return `Entendi. Vou te guiar como a Profesora Sofía.\n\nUma forma natural de praticar isso é criar frases curtas e corrigir aos poucos. Para seu nível ${nivel}, comece com estruturas simples e vocabulário útil.\n\nDesafio agora: ${desafios[Math.floor(Math.random() * desafios.length)]}`;
}
...
// ===== PRONUNCIA =====
const frasesPronuncia = [
  'Hola, me llamo Ana y soy de Brasil.',
  'El perro corre por la calle.',
  'Quisiera practicar español todos los días.',
  'La lluvia en Sevilla es una maravilla.',
  'Necesito reservar una mesa para dos.'
];

function gerarFrasesPronuncia() {
  const el = document.getElementById('pronunciaFrases');
  if (!el) return;
  el.innerHTML = frasesPronuncia.map(f => `<button onclick="selecionarFrasePronuncia('${f.replace(/'/g, "\\'")}')">${f}</button>`).join('');
}

function selecionarFrasePronuncia(frase) {
  const input = document.getElementById('pronunciaInput');
  if (input) input.value = frase;
  const destaque = document.getElementById('frasePronunciaDestaque');
  if (destaque) destaque.textContent = frase;
}

function falarTextoPronuncia() {
  const texto = document.getElementById('pronunciaInput')?.value.trim();
  if (!texto || !('speechSynthesis' in window)) { showToast('Recurso de voz indisponível neste navegador.'); return; }
...
function iniciarReconhecimentoPronuncia() {
  const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
  const result = document.getElementById('pronunciaResult');
  if (!SpeechRecognition) { result.textContent = 'Seu navegador não liberou reconhecimento de voz. Você ainda pode ouvir a guia e avaliar pelo texto.'; return; }
  const rec = new SpeechRecognition();
  rec.lang = 'es-ES';
  rec.interimResults = false;
  result.textContent = 'Ouvindo... fale a frase em espanhol.';
  rec.onresult = e => {
    const ouvido = e.results[0][0].transcript;
    result.dataset.ouvido = ouvido;
    result.innerHTML = `<strong>Reconhecido:</strong> ${ouvido}<br>Agora clique em avaliar.`;
  };
  rec.onerror = () => { result.textContent = 'Não consegui captar sua voz. Tente novamente em um ambiente mais silencioso.'; };
  rec.start();
}

function avaliarPronuncia() {
  const alvo = normalizarTexto(document.getElementById('pronunciaInput')?.value || '');
  const ouvido = normalizarTexto(document.getElementById('pronunciaResult')?.dataset.ouvido || '');
  const result = document.getElementById('pronunciaResult');
  if (!alvo) return;
  if (!ouvido) {
    result.innerHTML = 'Use o botão <strong>Gravar voz</strong> primeiro. Dica: fale devagar e abra bem as vogais.';
    return;
  }
  const alvoPalavras = alvo.split(' ').filter(Boolean);
  const ouvidoSet = new Set(ouvido.split(' ').filter(Boolean));
  const acertos = alvoPalavras.filter(p => ouvidoSet.has(p)).length;
  const score = Math.round((acertos / Math.max(1, alvoPalavras.length)) * 100);
  const dica = score > 80 ? 'Excelente ritmo. Agora tente falar um pouco mais natural.' : score > 55 ? 'Bom caminho. Repita focando nos sons de R, LL e vogais limpas.' : 'Vamos por partes: ouça a guia, repita devagar e grave novamente.';
  result.innerHTML = `<strong>${score}% de aproximação</strong><br>${dica}`;
  if (score >= 80) adicionarPontos(5);
}

function normalizarTexto(txt) {
  return txt.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^\wñ\s]/g, '').trim();
}

// ===== CORRIDA DE DIGITACAO =====
function renderizarDigitacaoSetup() {
  clearInterval(typingGame.timer);
  typingGame.iniciado = false;
  const root = document.getElementById('typingGameRoot');
  if (!root) return;
  root.innerHTML = `
    <div class="typing-setup">
      <button class="typing-back" onclick="navegar('inicio')">‹ Voltar</button>
      <h2>Corrida de Digitação</h2>
      <select class="typing-select" id="typingNivel">
        <option value="iniciante">Espanhol iniciante</option>
        <option value="intermediario">Espanhol intermediário</option>
        <option value="avancado">Espanhol avançado</option>
      </select>
      <div class="typing-racers-picker">
        ${typingPersonagens.map((p, i) => `
          <button class="typing-racer-option ${i === typingGame.personagem ? 'ativo' : ''}" onclick="selecionarTypingPersonagem(${i}, this)">
            ${p.icon}<small>${p.nome}</small>
          </button>
        `).join('')}
      </div>
      <button class="typing-start" onclick="iniciarCorridaDigitacao()">Começar</button>
    </div>
  `;
}

function selecionarTypingPersonagem(idx, btn) {
  typingGame.personagem = idx;
  document.querySelectorAll('.typing-racer-option').forEach(b => b.classList.remove('ativo'));
  btn?.classList.add('ativo');
}

function iniciarCorridaDigitacao() {
  typingGame = {
    ...typingGame,
    iniciado: true,
    fraseIdx: 0,
    ...
    progresso: 2,
    rivais: [14, 22, 30],
    nivel: document.getElementById('typingNivel')?.value || 'iniciante'
  };
  renderizarCorridaDigitacao();
  iniciarRivaisDigitacao();
  setTimeout(() => document.getElementById('typingInput')?.focus(), 80);
}

function renderizarCorridaDigitacao() {
  const root = document.getElementById('typingGameRoot');
  if (!root) return;
  const atual = obterFraseTyping();
  root.innerHTML = `
    <div class="typing-race">
      <div class="typing-track">
        <div class="typing-lane"></div><div class="typing-lane"></div><div class="typing-lane"></div><div class="typing-lane"></div>
        <div class="typing-finish"></div>
        ${renderizarCorredor('player', typingPersonagens[typingGame.personagem].nome, typingPersonagens[typingGame.personagem].icon, 72, typingGame.progresso)}
        ${renderizarCorredor('rival1', 'España', '🇪🇸', 150, typingGame.rivais[0])}
        ${renderizarCorredor('rival2', 'Flamenco', '💃', 236, typingGame.rivais[1])}
        ${renderizarCorredor('rival3', 'Fiesta', '🎉', 322, typingGame.rivais[2])}
      </div>
      <div class="typing-panel">
        <div class="typing-status">
          <span class="typing-badge">Comece a digitar</span>
          <div class="typing-stats">
            <span id="typingWpm">0 ppm</span>
            <span id="typingPrecisao">100% precisão</span>
            <span>${typingGame.fraseIdx + 1}/${typingFrases[typingGame.nivel].length}</span>
          </div>
        </div>
        <div class="typing-phrase" id="typingPhrase">${formatarFraseDigitacao(atual, '')}</div>
        <input class="typing-input" id="typingInput" autocomplete="off" spellcheck="false" placeholder="Digite a frase completa em espanhol..." oninput="atualizarCorridaDigitacao()" onkeydown="if(event.key==='Enter') finalizarFraseDigitacao()">
        <div class="typing-actions">
          <button class="primary" onclick="finalizarFraseDigitacao()">Confirmar frase</button>
          <button class="ghost" onclick="renderizarDigitacaoSetup()">Configurações</button>
          <button class="ghost" onclick="navegar('exercicios')">Ir para quizzes</button>
        </div>
      </div>
    </div>
  `;
...
function renderizarCorredor(classe, nome, icon, top, progresso) {
  return `
    <div class="typing-runner ${classe === 'player' ? 'player' : ''}" id="typing-${classe}" style="top:${top}px;left:${Math.min(88, progresso)}%">
      <span class="runner-label">${nome}</span>
      <span class="runner-icon">${icon}</span>
    </div>
  `;
}

function obterFraseTyping() {
  return typingFrases[typingGame.nivel][typingGame.fraseIdx] || typingFrases.iniciante[0];
}

function formatarFraseDigitacao(frase, digitado) {
  return frase.split('').map((ch, i) => {
    if (i >= digitado.length) return `<span>${ch}</span>`;
    return `<span class="${digitado[i] === ch ? 'ok' : 'bad'}">${ch}</span>`;
  }).join('');
}

function atualizarCorridaDigitacao() {
  const input = document.getElementById('typingInput');
  const frase = obterFraseTyping();
  if (!input) return;
  const digitado = input.value;
  const corretos = digitado.split('').filter((ch, i) => ch === frase[i]).length;
  const errosAgora = Math.max(0, digitado.length - corretos);
  typingGame.progresso = Math.min(88, 2 + ((typingGame.fraseIdx + corretos / frase.length) / typingFrases[typingGame.nivel].length) * 86);
  typingGame.erros = Math.max(typingGame.erros, errosAgora);
  document.getElementById('typingPhrase').innerHTML = formatarFraseDigitacao(frase, digitado);
  atualizarTypingStats();
  moverCorredores();
  if (digitado === frase) finalizarFraseDigitacao();
}

function atualizarTypingStats() {
  const minutos = Math.max(0.05, (Date.now() - typingGame.inicio) / 60000);
  const totalDigitado = Array.from(document.querySelectorAll('#typingInput')).reduce((sum, el) => sum + el.value.length, 0) + typingGame.acertos;
  const ppm = Math.round((totalDigitado / 5) / minutos);
  const precisao = Math.max(0, Math.round(((totalDigitado - typingGame.erros) / Math.max(1, totalDigitado)) * 100));
  const wpm = document.getElementById('typingWpm');
  const prec = document.getElementById('typingPrecisao');
  if (wpm) wpm.textContent = `${ppm} ppm`;
  if (prec) prec.textContent = `${precisao}% precisão`;
}

function finalizarFraseDigitacao() {
  const input = document.getElementById('typingInput');
  if (!input) return;
  const frase = obterFraseTyping();
  if (input.value !== frase) {
    showToast('Digite a frase completa exatamente como aparece.');
    return;
  }
  typingGame.acertos += frase.length;
  typingGame.fraseIdx++;
  if (typingGame.fraseIdx >= typingFrases[typingGame.nivel].length) {
    finalizarCorridaDigitacao();
    return;
  }
  input.value = '';
  document.getElementById('typingPhrase').innerHTML = formatarFraseDigitacao(obterFraseTyping(), '');
  document.querySelector('.typing-status .typing-stats span:last-child').textContent = `${typingGame.fraseIdx + 1}/${typingFrases[typingGame.nivel].length}`;
  atualizarCorridaDigitacao();
}

function iniciarRivaisDigitacao() {
  clearInterval(typingGame.timer);
  typingGame.timer = setInterval(() => {
    if (!typingGame.iniciado) return;
    typingGame.rivais = typingGame.rivais.map((p, i) => Math.min(86, p + [0.12, 0.10, 0.08][i]));
    moverCorredores();
  }, 220);
}

function moverCorredores() {
  const player = document.getElementById('typing-player');
  if (player) player.style.left = `${Math.min(88, typingGame.progresso)}%`;
  ['rival1','rival2','rival3'].forEach((id, i) => {
    const el = document.getElementById(`typing-${id}`);
    if (el) el.style.left = `${typingGame.rivais[i]}%`;
  });
}

function finalizarCorridaDigitacao() {
  clearInterval(typingGame.timer);
  typingGame.iniciado = false;
  typingGame.progresso = 90;
  moverCorredores();
  const tempo = Math.max(1, Math.round((Date.now() - typingGame.inicio) / 1000));
  const pontos = Math.max(20, 90 - typingGame.erros * 3);
  adicionarPontos(pontos);
  const root = document.getElementById('typingGameRoot');
  if (!root) return;
  root.innerHTML = `
    <div class="typing-setup typing-result">
      <h2>¡Meta alcançada!</h2>
      <p>Você completou a corrida em ${tempo}s e ganhou ${pontos} pontos.</p>
      <button class="typing-start" onclick="renderizarDigitacaoSetup()">Jogar novamente</button>
      <div class="typing-actions" style="justify-content:center">
        <button class="ghost" onclick="navegar('aulas-pronuncia')">Treinar pronúncia</button>
        <button class="ghost" onclick="navegar('conversacao')">Praticar conversa</button>
      </div>
    </div>
  `;
}
...
// ===== FEEDBACKS =====
const feedbacksBase = [
  { nome: 'Marina Lopes', nota: 5, texto: 'As aulas por nível deixaram meu estudo muito mais organizado.' },
  { nome: 'Rafael Costa', nota: 5, texto: 'O chat de conversação ajuda bastante a destravar frases do dia a dia.' },
  { nome: 'Bianca Souza', nota: 4, texto: 'Gostei muito da área de pronúncia e dos exercícios rápidos.' }
];

function obterFeedbacks() {
  return [...feedbacksBase, ...JSON.parse(localStorage.getItem('sw_feedbacks') || '[]')];
}

function gerarFeedbacks() {
  const lista = document.getElementById('feedbackList');
  if (!lista) return;
  lista.innerHTML = obterFeedbacks().reverse().map(f => `
    <div class="feedback-card">
      <div class="feedback-stars">${'★'.repeat(Number(f.nota))}${'☆'.repeat(5 - Number(f.nota))}</div>
      <p>${f.texto}</p>
...
function enviarFeedback() {
  const nome = document.getElementById('feedbackNome')?.value.trim() || usuarioAtual?.nome || 'Aluno Spanish World';
  const nota = document.getElementById('feedbackNota')?.value || '5';
  const texto = document.getElementById('feedbackTexto')?.value.trim();
  if (!texto) { showToast('Escreva seu feedback antes de enviar.'); return; }
  const salvos = JSON.parse(localStorage.getItem('sw_feedbacks') || '[]');
  salvos.push({ nome, nota, texto, data: new Date().toISOString() });
  localStorage.setItem('sw_feedbacks', JSON.stringify(salvos));
  document.getElementById('feedbackTexto').value = '';
  gerarFeedbacks();
  adicionarPontos(5);
  showToast('Feedback publicado. Obrigado!');
}

// ===== PERFIL =====
function gerarConteudoPerfil(aba, nivel, nivelPct) {
  const nomeCompleto = `${usuarioAtual?.nome || ''} ${usuarioAtual?.sobrenome || ''}`.trim();
  const conquistasLista = [
    ['primeira_aula','🎯','1ª Aula'], ['10_aulas','📚','10 Aulas'], ['50_aulas','🏅','50 Aulas'],
    ['100_aulas','🏆','100 Aulas'], ['100_pontos','🥉','100 pts'], ['500_pontos','🥇','500 pts'],
    ['chat_5','💬','Chat ativo'], ['top10','🎖️','Top 10']
  ];
  const telas = {
    'perfil-dados': `
      <div class="perfil-modal-wrap">
        <h2>👤 Meus dados</h2>
        <div class="perfil-grid">
          <div class="perfil-field"><label>Nome</label><input class="input-config" id="perfilNome" value="${usuarioAtual?.nome || ''}"></div>
          <div class="perfil-field"><label>Sobrenome</label><input class="input-config" id="perfilSobrenome" value="${usuarioAtual?.sobrenome || ''}"></div>
          <div class="perfil-field full"><label>Email</label><input class="input-config" id="perfilEmail" type="email" value="${usuarioAtual?.email || ''}"></div>
          <div class="perfil-field"><label>Nível</label><select class="input-config" id="perfilNivel">
            ${['iniciante','basico','intermediario','avancado','a1','a2','b1','b2','c1','c2'].map(n => `<option value="${n}" ${usuarioAtual?.nivel === n ? 'selected' : ''}>${traduzirNivel(n)}</option>`).join('')}
          </select></div>
          <div class="perfil-card"><strong>Membro desde</strong><br>${new Date(usuarioAtual?.criadoEm || Date.now()).toLocaleDateString('pt-BR')}</div>
        </div>
        <div class="perfil-actions"><button class="btn-config primary" onclick="salvarPerfilDados()">Salvar dados</button><button class="btn-config" onclick="fecharModalPerfil()">Fechar</button></div>
      </div>`,
    'perfil-personalizacao': `
      <div class="perfil-modal-wrap">
        <h2>🎨 Personalização</h2>
        <div class="perfil-field full"><label>Avatar</label><div class="avatar-picker">
          ${['🎓','🇪🇸','🌎','🔥','⭐','🎧','📚','💬','🏆','🚀'].map(a => `<button class="${usuarioAtual?.avatar === a ? 'ativo' : ''}" onclick="selecionarAvatarPerfil('${a}', this)">${a}</button>`).join('')}
        </div></div>
        <div class="perfil-grid" style="margin-top:18px">
          <div class="perfil-field"><label>Meta semanal</label><select class="input-config" id="perfilMeta"><option>3 aulas</option><option>5 aulas</option><option>10 aulas</option><option>20 aulas</option></select></div>
          <div class="perfil-field"><label>Modo de estudo</label><select class="input-config" id="perfilModo"><option>Equilibrado</option><option>Conversação</option><option>Gramática</option><option>Pronúncia</option></select></div>
        </div>
        <div class="perfil-actions"><button class="btn-config primary" onclick="salvarPersonalizacaoPerfil()">Salvar personalização</button></div>
      </div>`,
    'perfil-progresso': `
      <div class="perfil-modal-wrap">
        <h2>📊 Meu progresso</h2>
        <div class="perfil-grid">
          <div class="perfil-card"><strong>${pontosAtuais.toLocaleString()}</strong><br>Pontos</div>
          <div class="perfil-card"><strong>${streakAtual}</strong><br>Dias seguidos</div>
          <div class="perfil-card"><strong>${aulasVistas.length}</strong><br>Aulas concluídas</div>
          <div class="perfil-card"><strong>${conquistas.length}</strong><br>Conquistas</div>
        </div>
        <div class="perfil-card" style="margin-top:16px"><strong>Nível de evolução: ${nivel}</strong><div class="quiz-progress" style="margin:12px 0 0"><div class="quiz-progress-fill" style="width:${nivelPct}%"></div></div></div>
      </div>`,
    'perfil-conquistas': `
      <div class="perfil-modal-wrap">
        <h2>🏆 Conquistas</h2>
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:12px">
          ${conquistasLista.map(c => `<div class="perfil-card" style="text-align:center;opacity:${conquistas.includes(c[0]) ? 1 : .42}"><div style="font-size:30px">${c[1]}</div><strong>${c[2]}</strong></div>`).join('')}
        </div>
      </div>`,
    'perfil-seguranca': `
      <div class="perfil-modal-wrap"><h2>🔒 Segurança</h2>
        <div class="perfil-field"><label>Nova senha</label><input class="input-config" id="novaSenhaPerfil" type="password" placeholder="mínimo 8 caracteres"></div>
        <div class="config-item"><div class="config-item-info"><strong>Verificação em duas etapas</strong><small>Proteção extra para sua conta</small></div><button class="toggle-switch ${configuracoes.doisFatores ? 'ativo' : ''}" onclick="toggleConfig('doisFatores', this)"></button></div>
        <div class="perfil-actions"><button class="btn-config primary" onclick="salvarSenhaPerfil()">Atualizar senha</button></div>
      </div>`,
    'perfil-privacidade': `
      <div class="perfil-modal-wrap"><h2>🛡️ Privacidade</h2>
        ${[['publicoPerfil','Perfil público'],['mostrarPontos','Mostrar pontos no ranking']].map(i => `<div class="config-item"><div class="config-item-info"><strong>${i[1]}</strong><small>Você pode alterar a qualquer momento</small></div><button class="toggle-switch ${configuracoes[i[0]] ? 'ativo' : ''}" onclick="toggleConfig('${i[0]}', this)"></button></div>`).join('')}
      </div>`,
    'perfil-notificacoes': `
      <div class="perfil-modal-wrap"><h2>🔔 Notificações</h2>
        ${[['notificacoes','Alertas de estudo'],['emails','Receber emails'],['sonsAtivos','Sons da plataforma']].map(i => `<div class="config-item"><div class="config-item-info"><strong>${i[1]}</strong><small>Personalize seus avisos</small></div><button class="toggle-switch ${configuracoes[i[0]] ? 'ativo' : ''}" onclick="toggleConfig('${i[0]}', this)"></button></div>`).join('')}
      </div>`,
    'perfil-preferencias': `
      <div class="perfil-modal-wrap"><h2>⚙️ Preferências</h2>
        <div class="perfil-grid"><div class="perfil-field"><label>Interface</label><select class="input-config" id="prefIdioma"><option>pt-BR</option><option>es-ES</option></select></div><div class="perfil-field"><label>Velocidade da IA</label><select class="input-config" id="prefVelocidade"><option>normal</option><option>rápida</option><option>detalhada</option></select></div></div>
        <div class="config-item"><div class="config-item-info"><strong>Modo escuro</strong><small>Visual mais saturado e confortável à noite</small></div><button class="toggle-switch ${document.body.classList.contains('dark-mode') ? 'ativo' : ''}" onclick="alternarModoEscuro(); this.classList.toggle('ativo')"></button></div>
        <div class="perfil-actions"><button class="btn-config primary" onclick="salvarPreferenciasPerfil()">Salvar preferências</button></div>
      </div>`,
    'perfil-metas': `
      <div class="perfil-modal-wrap"><h2>🎯 Metas de estudo</h2>
        <div class="perfil-grid">
          <div class="perfil-card"><strong>Meta diária</strong><br>15 minutos de espanhol</div>
          <div class="perfil-card"><strong>Meta semanal</strong><br>Concluir 5 aulas e 2 quizzes</div>
          <div class="perfil-card"><strong>Foco atual</strong><br>Pronúncia e conversação</div>
          <div class="perfil-card"><strong>Próximo marco</strong><br>${Math.max(0, 10 - (aulasVistas.length % 10))} aulas para novo ciclo</div>
        </div>
        <div class="perfil-actions"><button class="btn-config primary" onclick="navegar('aulas-pronuncia');fecharModalPerfil()">Praticar agora</button><button class="btn-config" onclick="navegar('exercicios');fecharModalPerfil()">Fazer quiz</button></div>
      </div>`,
    'perfil-certificados': `
      <div class="perfil-modal-wrap"><h2>📜 Certificados</h2>
        <div class="perfil-card"><strong>Certificado A1</strong><br>${aulasVistas.length >= 20 ? 'Disponível para emissão' : `Conclua mais ${Math.max(0, 20 - aulasVistas.length)} aulas para liberar.`}</div>
        <div class="perfil-card" style="margin-top:12px"><strong>Certificado de conversação</strong><br>Envie 20 mensagens no Chat IA para desbloquear.</div>
        <div class="perfil-actions"><button class="btn-config primary" onclick="showToast('Área de emissão preparada para futura impressão em PDF.')">Ver detalhes</button></div>
      </div>`,
    'perfil-suporte': `
      <div class="perfil-modal-wrap"><h2>🧑‍💻 Suporte</h2>
        <div class="perfil-card"><strong>Email dos administradores</strong><br>sitespanishworld@gmail.com</div>
        <div class="perfil-card" style="margin-top:12px"><strong>Atalhos rápidos</strong><br>Use Contato para abrir uma mensagem pronta ou Feedbacks para registrar sua opinião.</div>
        <div class="perfil-actions"><button class="btn-config primary" onclick="navegar('contato');fecharModalPerfil()">Abrir contato</button><button class="btn-config" onclick="navegar('feedbacks');fecharModalPerfil()">Enviar feedback</button></div>
      </div>`,
    'perfil-assinatura': `
      <div class="perfil-modal-wrap"><h2>💎 Plano & Assinatura</h2>
        <div class="perfil-card"><strong>Plano atual: Spanish World Free</strong><br>Aulas, chat local, pronúncia, ranking e feedbacks liberados.</div>
        <div class="perfil-card" style="margin-top:12px"><strong>Plano Pro em breve</strong><br>Mentorias, certificados e trilhas profissionais.</div>
      </div>`
  };
  return telas[aba] || `<div class="perfil-modal-wrap"><h2>${nomeCompleto || 'Perfil'}</h2><p>Seção disponível em breve.</p></div>`;
}
...
function abrirAba(aba) {
  ...
  conteudo.innerHTML = gerarConteudoPerfil(aba, nivel, nivelPct);
  return;
...
function salvarPerfilDados() {
  if (!usuarioAtual) return;
  usuarioAtual.nome = document.getElementById('perfilNome')?.value.trim() || usuarioAtual.nome;
  usuarioAtual.sobrenome = document.getElementById('perfilSobrenome')?.value.trim() || '';
  usuarioAtual.email = document.getElementById('perfilEmail')?.value.trim() || usuarioAtual.email;
  usuarioAtual.nivel = document.getElementById('perfilNivel')?.value || usuarioAtual.nivel;
  salvarUsuario(); atualizarUIUsuario(); showToast('Perfil atualizado com sucesso!');
}

function selecionarAvatarPerfil(avatar, btn) {
  if (!usuarioAtual) return;
  usuarioAtual.avatar = avatar;
  document.querySelectorAll('.avatar-picker button').forEach(b => b.classList.remove('ativo'));
  btn?.classList.add('ativo');
  atualizarUIUsuario();
}

function salvarPersonalizacaoPerfil() {
  salvarUsuario(); atualizarUIUsuario(); showToast('Personalização salva!');
}

function salvarSenhaPerfil() {
  const nova = document.getElementById('novaSenhaPerfil')?.value || '';
  if (nova.length < 8) { showToast('A senha precisa ter pelo menos 8 caracteres.'); return; }
  usuarioAtual.senha = nova;
  salvarUsuario();
  showToast('Senha atualizada com segurança!');
}

function toggleConfig(chave, btn) {
  configuracoes[chave] = !configuracoes[chave];
  btn?.classList.toggle('ativo', configuracoes[chave]);
  salvarUsuario();
}

function salvarPreferenciasPerfil() {
  configuracoes.idiomInterface = document.getElementById('prefIdioma')?.value || configuracoes.idiomInterface;
  configuracoes.velocidadeBot = document.getElementById('prefVelocidade')?.value || configuracoes.velocidadeBot;
  salvarUsuario();
  showToast('Preferências salvas!');
}

function alternarModoEscuro() {
  themaDarkMode = !document.body.classList.contains('dark-mode');
  document.body.classList.toggle('dark-mode', themaDarkMode);
  localStorage.setItem('sw_darkmode', themaDarkMode ? 'true' : 'false');
  showToast(themaDarkMode ? 'Modo escuro ativado.' : 'Modo claro ativado.');
}
...
function enviarContato() {
  ...
  const assunto = document.getElementById('contatoAssunto')?.value || 'Contato Spanish World';
  const mensagem = document.getElementById('contatoMensagem')?.value.trim();
  if (!nome || !email || !mensagem) { showToast('Preencha nome, email e mensagem.'); return; }
  const body = encodeURIComponent(`Nome: ${nome}\nEmail: ${email}\n\n${mensagem}`);
  window.location.href = `mailto:sitespanishworld@gmail.com?subject=${encodeURIComponent(assunto)}&body=${body}`;
  showToast('Abrindo seu email para falar com os administradores.', 3000);
}
...
// ===== INICIALIZAÇÃO =====
function inicializarSpanishWorld() {
  if (window.__spanishWorldInicializado) return;
  window.__spanishWorldInicializado = true;
  preloadHeroImagens();
  inicializarHeroTematico();
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', inicializarSpanishWorld);
} else {
  inicializarSpanishWorld();
}
...
window.trocarHeroTema = trocarHeroTema;
...
window.renderizarDigitacaoSetup = renderizarDigitacaoSetup;
window.selecionarTypingPersonagem = selecionarTypingPersonagem;
window.iniciarCorridaDigitacao = iniciarCorridaDigitacao;
window.atualizarCorridaDigitacao = atualizarCorridaDigitacao;
window.finalizarFraseDigitacao = finalizarFraseDigitacao;
...
window.falarTextoPronuncia = falarTextoPronuncia;
window.iniciarReconhecimentoPronuncia = iniciarReconhecimentoPronuncia;
window.avaliarPronuncia = avaliarPronuncia;
window.selecionarFrasePronuncia = selecionarFrasePronuncia;
window.salvarPerfilDados = salvarPerfilDados;
window.selecionarAvatarPerfil = selecionarAvatarPerfil;
window.salvarPersonalizacaoPerfil = salvarPersonalizacaoPerfil;
window.salvarSenhaPerfil = salvarSenhaPerfil;
window.toggleConfig = toggleConfig;
window.salvarPreferenciasPerfil = salvarPreferenciasPerfil;
